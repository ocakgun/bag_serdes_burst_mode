...
def solve_casc_diff_dc(env_list,  # type: List[str]
                       ...
                       voutcm,  # type: float
                       vin_clip,  # type: float
                       clip_ratio_min,  # type: float
                       num_points=20,
                       inorm=1e-6,  # type: float
                       itol=1e-9  # type: float
                       ):
    # type: (...) -> Tuple[ List[np.ndarray], List[float], List[float]]
    ...
    for env, vbias, vload, vt, vm in zip(env_list, vbias_list, vload_list, vtail_list, vmid_list):
        ...
        for idx, vin_diff in enumerate([vin_clip, vin_max]):
        ...
        vout_max = vmat[1, 3] - vmat[1, 4]
        vout_clip = vmat[0, 3] - vmat[0, 4]
        cur_ratio = vout_clip / vout_max
        if cur_ratio < clip_ratio_min:
            # we didn't meet linearity spec, abort.
            # import pdb
            # pdb.set_trace()
            raise ValueError('clip_ratio = %.4g, failed linearity error spec at env = %s' % (cur_ratio, env))
        gain = vout_clip / vin_clip

        gain_list.append(gain)
        ratio_list.append(cur_ratio)
        vmat_list.append(vmat)

    return vmat_list, ratio_list, gain_list
...
def solve_casc_gm_dc(env_list,  # type: List[str]
    ...
    for env in env_list:
        ...
        idc_diff = ids_in - (idc_targ / inorm)  # type: DiffFunction
...
def characterize_casc_amp(env_list, lch, intent_list, fg_list, w_list, db_list, vbias_list, vload_list,
                          vtail_list, vmid_list, vincm, voutcm, vdd, vin_clip, clip_ratio_min,
                          cw, rw, fanout, ton, k_settle_targ, scale_min=0.25, scale_max=20):
    ...
    results = solve_casc_diff_dc(env_list, ndb, pdb, lch, intent_list, w_list, fg_list, vbias_list, vload_list,
                                 vtail_list, vmid_list, vdd, vincm, voutcm, vin_clip, clip_ratio_min, num_points=20)
...
def design_diffamp(root_dir, vin_clip, clip_ratio_min, gain_min, fanout, ton, k_settle_targ, rw, cw, env_range,
                   vdd, vincm, voutcm, lch, min_fg, fg_in, w_list, th_list,
                   root_dir_pmos=None, pmos_gd=False, fg_load_range=None,
                   fg_tail_range=None, fg_casc_range=None, ibias_range=None):
    tau_tail_max = ton

    if ibias_range is None:
        ibias_range = [50e-6, 75e-6, 100e-6]
    if fg_casc_range is None:
        fg_casc_range = list(range(6, 13, 2))
    if fg_tail_range is None:
            ...
            for fg_load in fg_load_range:
                ...
                try:
                    results = characterize_casc_amp(env_range, lch, th_list, fg_list, w_list, db_list, vbias_list,
                                                    vload_list, vtail_list, vmid_list, vincm, voutcm, vdd, vin_clip,
                                                    clip_ratio_min, cw, rw, fanout, ton,
                                                    k_settle_targ, scale_min=scale_min)
                except ValueError as ex:
                    print(ex)
                    continue

                vmat_list, ratio_list, gain_list, scale_list, cin_list = results
                ...
                print('min clip ratio: %.4g' % min(ratio_list))
                ...
                if gain_worst >= gain_min:
                    if opt_ibias is None or ibias_worst < opt_ibias:
                        ...
                        opt_info['ratio_list'] = ratio_list
...
def design_explore():
    ...
    pmos_gd = False
    spec_file = 'specs/diffamp_casc_gain_explore.yaml'
